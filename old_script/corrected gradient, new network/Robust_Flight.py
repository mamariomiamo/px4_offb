...
class MHE:
    ...
    def SetRotationVariable(self, rotation, rotation_h):
        self.R_B_v = rotation
        self.R_B_h = rotation_h
    ...
    def SetModelDyn(self, ode_mh, ode_ukf):
        ...
        self.MDyn_fn  = Function('MDyn', [self.state, self.ctrl, self.noise, self.R_B_v], [self.ModelDyn],
                                 ['s', 'c', 'n', 'R_B'], ['MDynf'])
        self.Modelukf = self.state + self.DT * ode_ukf
        self.MDyn_ukf = Function('MDynukf', [self.state, self.ctrl, self.R_B_h], [self.Modelukf],
                                 ['s', 'c', 'rbh'], ['MDynukff'])
...
class Auxiliary_MHE:
    def __init__(self, dim_xmhe, dim_state, dt_sample, xdot, ctrl_gain_v, n_ctrl_gain, ref_v, ex_v, ev_v, state_v, ctrl_v, u_v, xmhe_v, mass, J_B, R_B_v):
        ...
        weight_loss = exp(2*error)/sum1(exp(2*error))
    ...
    def AuxMHESolver(self, matA, matB, matD, matE, matF, matH, model_QR, track_e, Y, ctrl, ref_pva, x_hatmh):
        ...
        C_0  = np.matmul(LA.inv(np.identity(self.n_xmhe)+np.matmul(np.matmul(LA.inv(self.P0), np.transpose(self.H)), np.matmul(self.R, self.H))), LA.inv(self.P0))
        X_kf[0] = X_0
        C_k[0]  = C_0
        for k in range(self.horizon-1):
    ...
    def ChainRule(self, Ctrl_Gain, Y, ref, xmhe_traj, X_opt, R_b):
        ...
        weight = 75*np.exp(2*Loss_track) / np.sum(np.exp(2*Loss_track))

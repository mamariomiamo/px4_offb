...
class Controller:
    ...
    def position_ctrl(self, ctrl_gain, state, R_B ,ref_p, ref_v, ref_dv, b1_d, df_Imh):
        ...
        self.kax, self.kay, self.kaz = 0.4, 0.4, 0.4
        self.kwx, self.kwy, self.kwz = 0.4, 0.4, 0.4
        ...
        #fd = np.matmul(self.kp, e_x) + np.matmul(self.kv, e_v) - self.mass*ref_dv + df_Imh
...
class MHE:
    ...
    def SetArrivalCost(self, x_hatmh, P):
        assert hasattr(self, 'state'), "Define the state variable first!"
        self.P0 = P
        error_a = self.state - x_hatmh
        self.cost_a = 1/2*self.Gamma * mtimes(mtimes(transpose(error_a), inv(self.P0)), error_a)
    ...
    def SetCostDyn(self):
        ...
        assert hasattr(self, 'noise'), "Define the noise variable first!"
        # Tunable parameters
        R1, R2, R3, R4, R5, R6 = SX.sym('R1'), SX.sym('R2'), SX.sym('R3'), SX.sym('R4'), SX.sym('R5'), SX.sym('R6')
        # R7, R8, R9, R10, R11, R12 = SX.sym('R7'), SX.sym('R8'), SX.sym('R9'), SX.sym('R10'), SX.sym('R11'), SX.sym('R12')
        # R13, R14, R15, R16, R17, R18 = SX.sym('R13'), SX.sym('R14'), SX.sym('R15'), SX.sym('R16'), SX.sym('R17'), SX.sym('R18')
        Q1, Q2, Q3 = SX.sym('Q1'), SX.sym('Q2'), SX.sym('Q3')
        self.Gamma = SX.sym('gamma')
        self.R = diag(horzcat(R1, R2, R3, R4, R5, R6))
        self.Q = diag(horzcat(Q1, Q2, Q3))
        self.tunable_para = horzcat(R1, R2, R3, R4, R5, R6, Q1, Q2, Q3, self.Gamma)
        ...
        self.dJ_running = 1/2*(mtimes(mtimes(error_running.T, inv(self.R)), error_running) +
                               mtimes(mtimes(self.noise.T, inv(self.Q)), self.noise))
        self.dJ_fn = Function('dJ_running', [self.state, self.measurement, self.noise, self.tunable_para],
                              [self.dJ_running], ['s', 'm', 'n', 'tp'], ['dJrunf'])
        self.dJ_T  = 1/2*mtimes(mtimes(error_running.T, inv(self.R)), error_running)
        self.dJ_T_fn = Function('dJ_T', [self.state, self.measurement, self.tunable_para], [self.dJ_T],
                                ['s', 'm', 'tp'], ['dJ_Tf'])

    def para_QR(self, qr_p_t):
        P_min = 1e-6  # lower bound of parameter
        #P_min = 1e1  # lower bound of parameter
        P_max = 1e1  # upper bound of parameter
        gamma_min = 1e-5 # 1e-3, 5e-4
        gamma_max = 1e-1 # 5e-2
        qr_p = np.zeros((1, 9))
        for i in range(9):
        ...
        QR[0, -1] = gamma_min+(gamma_max-gamma_min)*qr_p_t[-1, 0] # the last is gamma
        return QR

    def MHEsolver(self, Y, track_e, x_hatmh, ctrl, model_QR, P, time, R_b, print_level=0):
        assert hasattr(self, 'state'), "Define the state variable first!"
        assert hasattr(self, 'noise'), "Define the noise variable first!"
        ...
        if time < time_mhe:
            ...
            self.horizon = int(time / self.DT) + 1
        else:
            # Moving horizon estimation
            self.horizon = self.N

        t_para = model_QR(track_e[len(track_e) - self.horizon])
        tunable_para = self.para_QR(t_para)
        if time < time_mhe:
            tunable_para[0, -1] = 1
        ...
        for k in range(self.horizon-1):
            ...
            t_para = model_QR(track_e[len(track_e)-self.horizon+k])
            tunable_para = self.para_QR(t_para)
            J    += self.dJ_fn(s=Xk, m=Y[len(Y)-self.horizon+k], n=Nk, tp=tunable_para)['dJrunf']
        ...
        tunable_para = self.para_QR(t_para)
        J += self.dJ_T_fn(s=Xk, m=Y[-1], tp=tunable_para)['dJ_Tf']
        ...
        for k in range(self.horizon-1):
            ...
            w_opt_bar = vertcat(0, 0, 0, 0, 0, 0, noise_traj_opt[k, 0], noise_traj_opt[k, 1],
                                noise_traj_opt[k, 2])
            t_para = model_QR(Y[len(Y) - self.horizon + k])
            tunable_para = self.para_QR(t_para)
            Q1, Q2, Q3 = tunable_para[0, 6], tunable_para[0, 7], tunable_para[0, 8]
            Q_bar = np.diag(np.array([1, 1, 1, 1, 1, 1, Q1, Q2, Q3]))
            costate_traj_opt[k:(k+1), :] = np.transpose(mtimes(LA.inv(mtimes(Q_bar, np.transpose(B_bar))), w_opt_bar))
    ...
    def lossMHE(self, Ground_truth, state_traj_opt):
        ...
        for k in range(self.horizon):
            loss += dloss_fn(gt=Ground_truth[len(Ground_truth)-self.horizon+k], Xmhe=state_traj_opt[k, :])['dlossf'].full()
        return loss
    ...
    def diffKKT(self, P):
        ...
        self.fu   = mtimes(mtimes(self.B, self.Q), mtimes(transpose(self.B), self.costate))
        self.D    = jacobian(self.fu, self.tunable_para)
        self.D_fn = Function('D', [self.costate, self.tunable_para], [self.D], ['cs', 'tp'], ['Df'])
        self.H_fn = Function('H', [self.state], [self.H], ['s'], ['Hf'])

        # Second-order derivative of Lagrangian
        self.dJrun_x = mtimes(mtimes(self.H.T, inv(self.R)), self.measurement - mtimes(self.H, self.state))
        self.E    = jacobian(self.dJrun_x, self.tunable_para)
        self.E_fn = Function('E', [self.state, self.measurement, self.tunable_para], [self.E], ['s', 'm', 'tp'], ['Ef'])
        self.dJ_x0= 1/self.Gamma * (mtimes(mtimes(mtimes(self.P0, self.H.T), inv(self.R)), self.measurement - mtimes(self.H, self.state)) + \
                    mtimes(mtimes(self.P0, transpose(self.A)), self.costate))
        self.F0   = jacobian(self.dJ_x0, self.tunable_para)
        self.F0_fn= Function('F0', [self.state, self.measurement, self.costate, self.tunable_para],
                             [self.F0], ['s', 'm', 'cs', 'tp'], ['F0f'])

    def GetAuxSys(self, state_traj_opt, costate_traj_opt, model_QR, Y, P):
        ...
        if horizon == 1:
            ...
            t_para = model_QR(curr_m)
            tunable_para = self.para_QR(t_para)
            matF = self.F0_fn(s=curr_s, m=curr_m, cs=curr_cs, tp=tunable_para)['F0f'].full()
            ...
            for t in range(horizon - 1):
                ...
                t_para = model_QR(curr_m)
                tunable_para = self.para_QR(t_para)
                matD += [self.D_fn(cs=curr_cs, tp=tunable_para)['Df'].full()]
                t_para = model_QR(next_m)
                tunable_para = self.para_QR(t_para)
                matE += [self.E_fn(s=next_s, m=next_m, tp=tunable_para)['Ef'].full()]
...
class Auxiliary_MHE:
    ...
    def SetPara(self, tunable_para):
        R1, R2, R3, R4, R5, R6 = tunable_para[0, 0], tunable_para[0, 1], tunable_para[0, 2], tunable_para[0, 3], tunable_para[0, 4], tunable_para[0, 5]
        # R7, R8, R9, R10, R11, R12 = tunable_para[0, 6], tunable_para[0, 7], tunable_para[0, 8], tunable_para[0, 9], tunable_para[0, 10], tunable_para[0, 11]
        # R13, R14, R15, R16, R17, R18 = tunable_para[0, 12], tunable_para[0, 13], tunable_para[0, 14], tunable_para[0, 15], tunable_para[0, 16], tunable_para[0, 17]
        self.R = np.diag(np.array([R1, R2, R3, R4, R5, R6]))
        Q1, Q2, Q3 = tunable_para[0, 6], tunable_para[0, 7], tunable_para[0, 8]
        self.Q = np.diag(np.array([Q1, Q2, Q3]))
        self.n_para = np.size(tunable_para)

    def para_QR(self, qr_p_t):
        P_min = 1e-6  # lower bound of parameter
        P_max = 1e1  # upper bound of parameter
        gamma_min = 7.5e-1 # 1e-3, 5e-4
        gamma_max = 1 # 5e-2
        qr_p = np.zeros((1, 9))
        for i in range(9):
            qr_p[0, i] = qr_p_t[i, 0]  # convert tensor to array
    ...
    def AuxMHESolver(self, matA, matB, matD, matE, matF, matH, P, model_QR, Y):
        ...
        t_para = model_QR(Y[len(Y) - self.horizon])
        tunable_para = self.para_QR(t_para)
        self.SetPara(tunable_para)
        if self.horizon < 10:
            tunable_para[0 ,-1] = 1
        self.P0 = 1/tunable_para[0, -1] * P
        ...
        X_0 = self.matF-np.matmul(LA.inv(np.identity(self.n_xmhe)+np.matmul(np.matmul(self.P0, np.transpose(self.H)), np.matmul(LA.inv(self.R), self.H))),
                                np.matmul(np.matmul(self.P0, np.transpose(self.H)), np.matmul(np.matmul(LA.inv(self.R), self.H), self.matF)))
        C_0  = np.matmul(LA.inv(np.identity(self.n_xmhe)+np.matmul(np.matmul(self.P0, np.transpose(self.H)), np.matmul(LA.inv(self.R), self.H))), self.P0)
        X_kf[0] = X_0
        C_k[0]  = C_0
        for k in range(self.horizon-1):
            t_para = model_QR(Y[len(Y) - self.horizon+1+k])
            tunable_para = self.para_QR(t_para)
            self.SetPara(tunable_para)
            X_kfp    = np.matmul(self.matA[k], X_kf[k]) + self.matD[k]
            P_next   = np.matmul(np.matmul(self.matA[k], C_k[k]), np.transpose(self.matA[k])) + np.matmul(np.matmul(self.matB, self.Q), np.transpose(self.matB))
            C_k[k+1]  = np.matmul(LA.inv(np.identity(self.n_xmhe)+np.matmul(np.matmul(P_next, np.transpose(self.H)), mtimes(LA.inv(self.R), self.H))), P_next)
            X_kf[k+1] = X_kfp - np.matmul(np.matmul(np.matmul(C_k[k+1], np.transpose(self.H)), np.matmul(LA.inv(self.R), self.H)), X_kfp) + np.matmul(C_k[k+1], self.matE[k])
        ...
        for i in range((self.horizon-1), 0, -1):
            if i == self.horizon-1:
                ...
                tunable_para = self.para_QR(t_para)
                self.SetPara(tunable_para)
                Lambda_pre = -np.matmul(np.matmul(np.transpose(self.H), LA.inv(self.R)), np.matmul(self.H, X_kf[i])) + self.matE[i-1]
            else:
                t_para = model_QR(Y[len(Y) - self.horizon+i])
                tunable_para = self.para_QR(t_para)
                self.SetPara(tunable_para)
                Lambda_pre = np.matmul(np.identity(self.n_xmhe) - np.matmul(np.matmul(np.transpose(self.H), LA.inv(self.R)), np.matmul(self.H, C_k[i])), np.matmul(np.transpose(self.matA[i]), LAMBDA[i])) - \
                             np.matmul(np.matmul(np.transpose(self.H), LA.inv(self.R)), np.matmul(self.H, X_kf[i])) + self.matE[i - 1]
    ...
    def loss_tracking(self, ref, state):
        ...
        loss_track = loss_fn(s=state, ref=ref)['lossf'].full()
        return loss_track
    ...
    def ChainRule(self, Ctrl_Gain, Y, ref, xmhe_traj, X_opt, R_b):
        ...
        for t in range(self.horizon-2):
            ...
            x_mhe[9:21, 0] = Y[len(Y)-self.horizon+t][6:18, 0]
            # Compute the loss

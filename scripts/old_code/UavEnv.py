...
class quadrotor:
    def __init__(self, para, IGE_para):
    ...
    def Aerodynamics(self, state, ctrl, constant_dis, P_xy0, payload_mass, tether_length, aero_para):
        if constant_dis:
            dis = aero_para
        else:
            # IGE disturbance
            ...
            take_off = np.array([[P_xy0[0], P_xy0[1], -0.1]]).T

            Position = take_off - P_com
            origin   = np.array([[0, 0, -0.1]]).T
            if LA.norm(Position)<=tether_length:
                disf_payload = np.zeros((3, 1))
                Disf_total = disf_payload+Disf_i
                dis = np.array([[Disf_total[0, 0], Disf_total[1, 0], Disf_total[2, 0], distx_b[0], disty_b[0], distz_b[0]]]).T
            elif LA.norm(Position)<=2*tether_length:
                Position_o = origin - P_com
                direction = Position_o/LA.norm(Position_o)
                disf_payload = direction*f_payload
                Disf_total = disf_payload + Disf_i
                dis = np.array([[Disf_total[0, 0], Disf_total[1, 0], Disf_total[2, 0], distx_b[0], disty_b[0], distz_b[0]]]).T
            else:
                Position_o = origin - P_com
                direction = Position_o / LA.norm(Position_o)
                disf_payload = direction * 2* f_payload
                Disf_total = disf_payload + Disf_i
                dis = np.array([[Disf_total[0, 0], Disf_total[1, 0], Disf_total[2, 0], distx_b[0], disty_b[0], distz_b[0]]]).T
